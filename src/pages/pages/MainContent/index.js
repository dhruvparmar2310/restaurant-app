...
export default function MainContent () {
  ...
  const handlecart = (e, sizeData, popUpDetails, optionData, name, id, filterProduct) => {
    setToggle(false)
    const sum = optionData.price + sizeData.price
    console.log('sum :>> ', sum)
    // const IteamIndex = cartData.findIndex((iteam) => console.log('iteam :>> ', iteam))
    const cart = setCart(sizeData, popUpDetails, optionData, name, popUpDetails.id, filterProduct)
    console.log('cart :>> ', cart.payload)
    const fProduct = cart.payload.filter((data) => data.filterProduct.map((data, index) => {
    ...
    console.log('fProcut >> ', fProduct)
    if (cart.payload) {
      if (cart.payload.id === popUpDetails.id) {
        // console.log(cart)
        dispatch(cart)
      }
    } else {
        ...
        {toggle
          ? <>
            <div className='pop-up'>
              ...
              <button className='addOrder mt-3 btn' onClick={(e) => handlecart(e, sizeData, popUpDetails, optionData, name, popUpDetails.id, filterProduct)}>Add to Order</button>
            </div>
            </>
          : ''}
    </div>
